        ...
        ColumnLayout {
            ...
            RowLayout {
                ...
                ML4WButton { 
                    ...
                    onClicked: {
                        root.isOpen = false
                        Quickshell.execDetached(["bash", "-c", "qs ipc call settings toggle"])
                    }
                }
                ML4WButton { 
                    text: "Hyprland"
            ...
            ScrollView {
                ...
                ColumnLayout {
                    ...
                    RowLayout {
                        Layout.fillWidth: true
                        Text { text: "Catppuccin"; color: Theme.on_background; font.family: Theme.fontFamily; font.pixelSize: 16 }
                        Item { Layout.fillWidth: true }
                        ActionIcon { 
                            iconTxt: "󰔎"
                            onClicked: {
                                Quickshell.execDetached(["bash", "-c", Quickshell.env("HOME") + "/.config/catppuccin/theme-switcher.sh"])
                            }
                        }
                        SettingsWheel {
                            onClicked: catppuccinMenu.open()
                            Menu {
                                id: catppuccinMenu
                                y: parent.height
                                implicitWidth: 220
                                padding: 8
                                background: Rectangle { color: Theme.background; border.color: Theme.primary; border.width: 1; radius: 8 }
                                ML4WMenuItem { text: "Latte (Light)"; onClicked: {
                                        Quickshell.execDetached(["bash", "-c", Quickshell.env("HOME") + "/.config/catppuccin/theme-switcher.sh latte"])
                                    }
                                }
                                ML4WMenuItem { text: "Frappe (Dark)"; onClicked: {
                                        Quickshell.execDetached(["bash", "-c", Quickshell.env("HOME") + "/.config/catppuccin/theme-switcher.sh frappe"])
                                    }
                                }
                                ML4WMenuItem { text: "Macchiato (Dark)"; onClicked: {
                                        Quickshell.execDetached(["bash", "-c", Quickshell.env("HOME") + "/.config/catppuccin/theme-switcher.sh macchiato"])
                                    }
                                }
                                ML4WMenuItem { text: "Mocha (Dark)"; onClicked: {
                                        Quickshell.execDetached(["bash", "-c", Quickshell.env("HOME") + "/.config/catppuccin/theme-switcher.sh mocha"])
                                    }
                                }
                                ML4WMenuItem { text: "Wallust (Dynamic)"; onClicked: {
